...
var OneBook3D = {
	...
	set_options:function(options){
		...
		if(typeof(options)==='number' ){
		   switch(options){
		   case 1: options = {flip:'soft',border:25};break;
		   case 2: options = {skin:'light',slope:1};break;   
		   case 3: options = {skin:'light',flip:'soft',startPage:6};break;   
	...
	toggleSkin:function(){
		var 
			...
			hlp_update = function(){
				_this.$gHelpLayer.find('span[name=home_link]')
				.css({background:'url('+SKIN.HOME_LINK[invertMode].toDataURL()+') no-repeat center'});
				_this.$gHelpLayer.find('span[name=help_arrow]')
				.css({background:'url('+SKIN.HELP_ARROW[invertMode].toDataURL()+') no-repeat center'});
			};
			...
			this.$gHelpLayer.find('.hlpLeftSide').css({background:'url('+bgImage+')'});
			this.$gHelpLayer.find('.hlpRightSide').css({background:'url('+bgImage+')'});
			hlp_update();
			btn_update();
		}else{
			this.DARK_MODE = true;
	...
	rotate_x_to:function(degree){
		...
		this.$gContainter.css({'-webkit-transform':transform,'-moz-transform':transform});
	},
	...
	fit3dDataToBookSize:function(){
		...
		var kunscaled = bookUnscaled.width/dataWidth;
		...
		calculate(this.FRAMES_UNSCALED[0],kunscaled);
		calculate(this.FRAMES_UNSCALED[1],kunscaled);

		this.initWithSize(bookSize,bookUnscaled);
	...
	initWithSize:function(bookSize,bookUnscaled){
		...
		this.$gContainter.css({
				...
				height:this.CFG.BOOK_SIZE.height*this.CFG.ROTATE_CENTER_OFFSET,border:'0px solid none'
				});
	...
	moveSheetTo:function(frame,param,DBG_NAME){
		var _this = this,
			...
			edgeOffset = 2, 
			edges3d = this.FRAMES[forward][frame].edges3d,
	...
	getSheetByIndex:function(sheetIndex){		
		return this.$ARR_SHEETS[sheetIndex];
	},	
	...
	animateSheet:function(direction){
		...
		var 
			...
			$sheet = this.getSheetByIndex(actualSheet),
	...
	switchCurrents:function(SHEET_PARAM){
		...
		$wholeimage.css({'-webkit-transform':'rotateY('+gr+'deg)','-moz-transform':'rotateY('+gr+'deg)'});
						
		this.CURRENT.spread += forward?1:-1;
		this.CURRENT.frame = 0;
		this.START_PAGE = _this.CURRENT.spread*2?_this.CURRENT.spread*2:1;
		
		this.bookLoaderShow(!this.didSpreadLoaded(sheet));
		...
		if (_this.ENVIRON.chrome){
			...
			if(orderDirection && this.isDirectionCorrect(orderDirection)){			
				$wholeimage.fadeIn('normal',function(){
					_this.SHEET_DISPLAYED = null;
					$all_edges.hide();
					_this.ORDER_TO_ANIMATE = false;
					_this.animateSheet(orderDirection);	
	...
	didSpreadLoaded:function(spread){
		var $pageLeft = this.CURRENT.spread>0?this.$gContainter.find('.sheet'+(this.CURRENT.spread-1)):false; 
		var $pageRight = this.CURRENT.spread<this.ALLSHEETS.length?this.$gContainter.find('.sheet'+this.CURRENT.spread):false; 
		if($pageLeft){
	...
	buildHelpLayer:function(){
		var 
			...
			lngHlpGoToHomelink = this.getLNG('#'),
		...
		if(!sizeMini){hlpStart.push('<a style="display:table;margin:0px 0px 0px auto;" href="#" title=""><span name="home_link" style="background:url('+SKIN.HOME_LINK[skinMode].toDataURL()+') no-repeat center;"></span></a>')};
	...
	buildAndPreloadSheets:function(){
		...
		var fn = {
			buildColorSheet:function($sheet,colors,num_sheet){
				...
				for(var m=0;m<edges3d.length;m++){
					...
					var transformBackface = '-webkit-transform:rotateY(-180deg);-moz-transform:rotateY(-180deg);';
				...
				var wholeimage = [					
					'<span class="back_image" style="-webkit-transform:rotateY(-180deg);-moz-transform:rotateY(-180deg);"><div style=""></div></span>',
				...
				$sheet.append([
					...
					'-webkit-transform:translate3d(0px,0px,0px) rotateY('+gr+'deg);-moz-transform:translate3d(0px,0px,0px) rotateY('+gr+'deg);">'+wholeimage+'</span>'
				].join(''));				
			},
		...
		var buildSheets = function(n){	
			...
			var $sheet = $([
				...
				'style="-webkit-transform:translate3d(0px,0px,'+zindex3d+'px);-moz-transform:translate3d(0px,0px,'+zindex3d+'px);',
		...
		var fn = {
			...
			cutImagesAndInsertIntoEdges:function(img){
				...
				for(var i=0;i<edges3d.length;i++){
					...
					canvas.getContext('2d').drawImage(img,left,0,cropWidth,imgHeight,0,0,cropWidth,imgHeight);
